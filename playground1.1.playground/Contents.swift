...
//  prog1.1
//
//  Created by Christopher Clarke on 7/11/18.
 ...
 * @return: String - utf code for matched emoji or
 * the original string
 */
 ...
 * @param: str - display
 *
 * @return: String - argument string with some words
 * replaced with utf code for corresponding emojis.
 */
func emojiate(str: String) -> String {
    // Break the string into seperate
    ...
    for char: Character in str {
        ...
        if(String(char) >= "A" && String(char) <= "z"){
            ...
            // Add the current non-letter character to the
            // result (swift doesn't allow addition
            // if Character to String, so use append
    ...
    // (matching it with emoji of possible)
    result += emojicode(word: word)
